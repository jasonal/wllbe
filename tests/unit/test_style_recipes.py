...
def test_load_recipe_catalog_reads_every_recipe(tmp_path: Path) -> None:
    ...
    _write_json(
        recipe_dir / "neutral-balanced.json",
        {
            ...
            "preferred_style_pack_id": "clean-light",
        },
    )
    _write_json(
        recipe_dir / "bold-contrast.json",
        {
            ...
            "preferred_style_pack_id": "dark-tech",
        },
    )
...
def test_build_version_plan_pairs_recipes_and_styles(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert plan[0].recipe_id == "alpha-card"
    assert plan[0].style_pack_id == "alpha"
    assert plan[1].version_id == "version-b"
    assert plan[1].recipe_id == "zebra-roll"
    assert plan[1].style_pack_id == "zebra"
    assert plan[1].allowed_layout_families == ("panel",)
    assert plan[1].motion_intensity == "high"
...
def test_build_version_plan_with_actual_catalog_files() -> None:
    ...
    assert plan[0].style_pack_id == "dark-tech"
    assert plan[1].style_pack_id == "clean-light"
...
def test_build_version_plan_ignores_cwd(tmp_path: Path) -> None:
    original_cwd = Path.cwd()
    new_dir = tmp_path / "sandbox"
    new_dir.mkdir()
    os.chdir(new_dir)
    try:
        plan = recipes.build_version_plan(2)
...
def test_load_recipe_catalog_rejects_bad_layout(tmp_path: Path) -> None:
    recipe_dir = tmp_path / "recipes"
    recipe_dir.mkdir()
    _write_json(
        recipe_dir / "bad-layout.json",
        {
            "recipe_id": "bad-layout",
            "label": "Bad Layout",
            "allowed_layout_families": "not-a-list",
            "motion_intensity": "low",
            "density_target": "medium",
            "preferred_style_pack_id": "clean-light",
        },
    ...
    with pytest.raises(ValueError):
        catalog.load_recipe_catalog(recipe_dir)
